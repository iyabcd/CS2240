...
app.post('/subjects/save', [
    check('subjeect_title').isLength({min:1}).matches(/^[A-Za-z\s]+$/),
    check('subject_number').isLength({min:1}).matches(/^[a-zA-Z0-9]+$/),
    ...
    if(!errors.isEmpty()){
        const alert = errors.array();
        req.flash('message', 'Input must be LETTERS only for TITLE, LETTERS AND NUMBERS for NUMBER, and NUMBERS ONLY for LOADS. <br> Fields should not be empty. Please try again!');
        req.flash('message', 'Input must be LETTERS only for TITLE, LETTERS AND NUMBERS for NUMBER, and NUMBERS ONLY for LOADS. <br> Fields should not be empty. Please try again!');
        ...
        let query = connection.query(sql, data,(err, results) => {
        if(err) throw err;
        req.flash('message', 'Subject "' + req.body.subject_title + ' - ' + req.body.subject_number + '" successfully added!');
        res.redirect('/subjects');
        });
    }
});
