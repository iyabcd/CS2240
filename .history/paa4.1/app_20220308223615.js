...
app.get('/teachers/delete/:teacher_id',(req, res) => {
    ...
    let sql_name = connection.query(`SELECT teacher_lname FROM teachers WHERE teacher_id = ${teacher_id}`,(err, results) => {
        if(err) throw err;
        teacher_lname = results[0].teacher_lname;
        console.log(teacher_lname);
        console.log(results[0].teacher_lname);
    });
    let sql = `DELETE FROM teachers WHERE teacher_id = ${teacher_id}`;
    let query = connection.query(sql,(err) => {
        if(err) throw err;
        req.flash('message', + teacher_lname +' successfully deleted!');
        res.redirect('/');
    });
});
//search teacher
app.get('/teachers/search/', (req, res) => {
    ...
    console.log(term);
    let sql = `SELECT * FROM teachers WHERE teacher_id LIKE ${term}`;
    let query = connection.query(sql, (err,rows) => {
        if(err) throw err;
        if(rows.length < 1){
            req.flash('message', 'No result/s.');
            res.redirect("/");
        }else {
            res.render('teacher_index', {
                title : 'Teacher Management System',
