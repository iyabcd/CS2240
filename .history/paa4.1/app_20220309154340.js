...
app.post('/teachers/save', [
    check('teacher_lname').isLength({min:3}).matches(/^[A-Za-z\s]+$/),
    check('teacher_fname').isLength({min:3}).matches(/^[A-Za-z\s]+$/),
    check('teacher_mname').isLength({min:3}).matches(/^[A-Za-z\s]+$/)
], (req, res) => {
    const errors = validationResult(req);
    if(!errors.isEmpty()){
        const alert = errors.array();
        req.flash('message', JSON.stringify(alert[0]));
        res.redirect('/');
    }else{
