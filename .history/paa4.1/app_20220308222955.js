...
app.get('/teachers/delete/:teacher_id',(req, res) => {
    ...
    let sql = `SELECT teacher_lname FROM teachers WHERE teacher_id = ${teacher_id}; DELETE FROM teachers WHERE teacher_id = ${teacher_id}`;
    let query = connection.query(sql,(err) => {
        if(err) throw err;
        req.flash('message', result[0].teacher_lname + ' successfully deleted!');
        res.redirect('/');
    });
});
//search teacher
