...
app.get('/teachers/delete/:teacher_id',(req, res) => {
    ...
    let name = connection.query(`SELECT * FROM teachers WHERE teacher_id = ${teacher_id}`);
    console.log(name.teacher_fname);
    let sql = `DELETE FROM teachers WHERE teacher_id = ${teacher_id}`;
    let query = connection.query(sql,(err) => {
        if(err) throw err;
        req.flash('message', name.teacher_lname + ' successfully deleted!');
        res.redirect('/');
    });
});
//search teacher
