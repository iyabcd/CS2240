...
app.post('/teachers/save', [
    check('teacher_lname').isLength({min:2}).matches(/^[A-Za-z\s]+$/),
    check('teacher_fname').isLength({min:2}).matches(/^[A-Za-z\s]+$/),
    check('teacher_mname').isLength({min:2}).matches(/^[A-Za-z\s]+$/)
], (req, res) => {
