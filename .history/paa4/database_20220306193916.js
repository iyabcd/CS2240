...
app.post('/teachers', (req,res)=>{
    var sql = "SET @teacher_id = ?; SET @teacher_lname = ?; SET @teacher_fname = ?; SET @teacher_mname = ?; CALL TeacherAddOrEdit(@teacher_id,@teacher_lname,@teacher_fname,@teacher_mname)"
    mysqlConnection.query('DELETE FROM teachers WHERE teacher_id = ?',[req.params.id], (err,rows,fields)=>{
        if(!err){
            res.send('Teacher is deleted.');
        }else{
