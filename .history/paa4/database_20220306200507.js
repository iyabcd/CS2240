...
app.post('/teachers', (req,res)=>{
    let teach = req.body;
    var sql = " SET @teacher_lname = ?; SET @teacher_fname = ?; SET @teacher_mname = ?; \
    CALL TeacherAddOrEdit(@teacher_lname,@teacher_fname,@teacher_mname);";
    mysqlConnection.query(sql, [teach.teacher_lname, teach.teacher_fname, teach.teacher_mname], (err,rows,fields)=>{
        if(!err){
            res.send(rows);
        }else{
            console.log(err);
